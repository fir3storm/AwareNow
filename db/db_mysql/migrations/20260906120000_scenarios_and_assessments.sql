...
-- convention already used elsewhere in this repo (see 20260905120000).
CREATE TABLE IF NOT EXISTS `scenarios` (
    ...
    `created_at`                  DATETIME,
    INDEX `idx_scenarios_owner_id` (`owner_id`),
    INDEX `idx_scenarios_source_reported_message_id` (`source_reported_message_id`)
);
...
CREATE TABLE IF NOT EXISTS `assessments` (
    ...
    `status`                    VARCHAR(64) NOT NULL,
    `created_at`                DATETIME,
    INDEX `idx_assessments_owner_id` (`owner_id`)
);
